...
⟨closure_subset_adjoin_root K f, by apply_instance,
begin
  introsI,
  show _ = adjoin_root_lift K f _ _,
...
⟨λ f hf0, let ⟨g, hg⟩ := is_noetherian_ring.exists_irreducible_factor
  ...
  begin
    ...
    cases classical.some_spec (exists_algebraic_closure K)
      (adjoin_root_extension K g) (closed_extension_le_adjoin_root_extension K g)
        with hsub hom,
    letI := hom.1,
    have left_inv : left_inverse (inclusion hsub ∘ (equiv.set.range _
      (adjoin_root_extension_map K g).2)) adjoin_root.of,
      from λ _, subtype.eq $ begin
          rw [function.comp_app, equiv.set.range_apply, inclusion],
          dsimp,
          rw [adjoin_root_extension_map_apply],
        end,
    let e : algebraic_closure K ≃r adjoin_root g :=
      { to_fun := adjoin_root.of,
        inv_fun := inclusion hsub ∘ (equiv.set.range _ (adjoin_root_extension_map K g).2),
        left_inv := left_inv,
